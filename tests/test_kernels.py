...
def calc_variance_from_kernels(vals, add_empty_entries=True, pre_accumulate_idx_l=[]):
    if len(pre_accumulate_idx_l) != 0:
        ...
        return _Variance.consolidate_stats(*_prep_entries(vals, add_empty_entries))
...
def _test_consolidate_variance(vals, mean_rtol=0.0, variance_rtol=0.0):
    ...
    for pre_accumulate_idx_l in pre_accumulate_idx_l_vals:
        ...
        np.testing.assert_array_equal(ref_result["counts"], actual_result["counts"])
        np.testing.assert_allclose(
            ref_result["mean"], actual_result["mean"], atol=0.0, rtol=mean_rtol
        )
        np.testing.assert_allclose(
            ref_result["variance"],

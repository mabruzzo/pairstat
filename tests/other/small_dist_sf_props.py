...
def _cal_ref_props(
    dist_bin_edges, component_fields, cut_regions, geometric_selector, statistic, kwargs
):
    ...
    for cr_ind, cut_region in enumerate(cut_regions):
        ...
        pos = np.array(
            [cad[ii].to("cloud_radius").ndarray_view() for ii in ["x", "y", "z"]]
        )
        quan = np.array(
            [cad[f].to("wind_velocity").ndarray_view() for f in component_fields]
        )
...
def compare(
    dist_bin_edges, cut_regions, geometric_selector, nproc=1, statistic="histogram"
):
    component_fields = (
        ("gas", "velocity_x"),
        ("gas", "velocity_y"),
    ...
    for stat_name in statistic_l:
        if stat_name == "histogram":
            vel_bin_edges = np.array(
                [0.0]
                + np.geomspace(1e-10, 1.0, num=101).tolist()
...
def compare_variance(
    ref, actual, mean_rtol=0.0, variance_rtol=0.0, mean_atol=0.0, variance_atol=0.0
):
    for key, r_vals, a_vals in _kv_pair_cmp_iter(ref, actual):
        if key == "counts":
            np.testing.assert_array_equal(
                r_vals, a_vals, err_msg="the 'counts' entries should be identical"
            )
            continue
        elif key == "mean":
...
def perform_comparison(statistic_l, compare_func, ref, actual, kwargs={}):
    if isinstance(statistic_l, str):
        compare_func(statistic=statistic_l, ref=ref, actual=actual, **kwargs)
    else:
        for i, statistic in enumerate(statistic_l):
            assert isinstance(statistic, str)
            print("Comparing: " + statistic)
            compare_func(statistic=statistic, ref=ref[i], actual=actual[i], **kwargs)
...
def compare_rslts(
    ...
    mean_atol=0.0,
    variance_atol=0.0,
    weight_total_rtol=0.0,
    weight_total_atol=0.0,
    baverage_rtol=0.0,
    baverage_atol=0.0,
    bvariance_rtol=0.0,
    bvariance_atol=0.0,
    ...
    for i in range(len(ref)):
        if statistic == "histogram":
            ...
            compare_bulkstat(
                ...
                average_rtol=baverage_rtol,
                average_atol=baverage_atol,
                variance_rtol=bvariance_rtol,
                variance_atol=bvariance_atol,
...
def test_single_subvol(statistic):
    ...
    my_geometric_selector = BoxSelector(
        left_edge=[-2.0, -2.0, -2.0],
        right_edge=[2.0, 2.0, 2.0],
    ...
    ref, actual, points_used, subvol_decomp = compare(
        my_dist_bin_edges, cur_cut_regions, my_geometric_selector, statistic=statistic
    )
    ...
    for dim in range(3):
        ...
        perform_comparison(
            ...
            kwargs={
                "mean_rtol": 0.0,
                "variance_rtol": 0.0,
                "mean_atol": [5.0e-16, 3e-16, 3e-16][dim],
                "variance_atol": [1e-17, 2e-17, 8e-18][dim],
                "weight_total_rtol": 3e-16,
                "weight_total_atol": 0.0,
                "baverage_rtol": 0.0,
                "baverage_atol": 2e-17,
                "bvariance_rtol": 0.0,
                "bvariance_atol": 2e-18,
            },
        )
    ...
    for i in range(3):
        ...
        perform_comparison(
            ...
            kwargs={
                "mean_rtol": 0.0,
                "variance_rtol": 0.0,
                "mean_atol": [5.0e-16, 5e-16, 5e-16][i],
                "variance_atol": [9e-18, 9e-18, 2e-17][i],
                "weight_total_rtol": 2e-16,
                "weight_total_atol": 0,
                "baverage_rtol": 0.0,
                "baverage_atol": 7.0e-18,
                "bvariance_rtol": 0.0,
                "bvariance_atol": 1.0e-18,
            },
        )

...
def setup_ds():
    ...
    path = (
        "/media/mabruzzo/Elements/turb_cloud/cloud_runs/"
...
def _yt_calc_bulkstat(
    fields, ds, geometric_selector, cut_regions, stat_kw_pairs, quantity_units="cm/s"
):
    ...
    for cr_ind, cut_region in enumerate(cut_regions):
        ...
        rslt = {
            ...
            "average": np.empty((n_weight_fields, len(fields)), dtype=np.float64),
        }
        ...
        for j, (weight_field, weight_units) in enumerate(kw["weight_field"]):
            ...
            if stat_name == BulkAverage.name:
                tmp = cad.quantities.weighted_average_quantity(
                    list(fields), weight_field
                )
...
def compare_bulk_stat(
    stat_kw_pair, ds, force_subvols_per_ax=(1, 1, 1), **compare_kwargs
):
    # force_subvols_per_ax only affects the call to small_dist_sf_props
    my_geometric_selector = BoxSelector(
        left_edge=[-4.0, -4.0, -4.0],
        right_edge=[4.0, 4.0, 4.0],
    ...
    bin_edges = np.array([5.0 / 18.0, 5.0 / 9.0, 5.0 / 6.0, 10.0 / 9.0, 25.0 / 18.0])
    my_cut_regions = [
    ...
    for i in range(len(my_cut_regions)):
        compare_bulkstat(
            alt_rslt_l[0][i],
            actual_rslt_l[0][i],
            bulkvariance_cmp=bulkvariance_cmp,
            **compare_kwargs,
        )

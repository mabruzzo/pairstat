...
neighbor_vel_differences = functools.partial(
    ...
    field_components = [('gas', 'velocity_x'),
                        ('gas', 'velocity_y'),
                        ('gas', 'velocity_z')]
)
...
max_neighboring_cs = functools.partial(
    _apply_neighbor_ops_to_field,
    field = ('gas', 'sound_speed'),
    op = np.maximum
)

def get_shared_logXe_bin_ind(grid, axis,
                             logX_e_bin_edges):
    """
...
def _reference_calculation(ds, logX_e_bin_edges, hist_bin_edges):
    ...
    name_pairs = [('parallel', 'aligned_vdiff'),
...
def func(ds, phase_bin_edges):
    ...
    my_cut_regions.append(
        f'(obj["logX_e"].v > {float(phase_bin_edges[-1]):.15e})'
    )
    ...
    for cr_ind, actual_counts_dict in enumerate(actual_results):
        ...
        for key in ['aligned_vdiff_counts', 'transverse_vdiff_counts',
                    'mag_vdiff_counts']:
...
if __name__ == '__main__':
    ...
    if False:
        ...
        hist_edges = {'aligned_vdiff_edges' : aligned_edges,
                      'transverse_vdiff_edges' : transverse_edges,
                      'mag_vdiff_edges' : transverse_edges.copy()}

        _reference_calculation(ds, logX_e_bin_edges = phase_bin_edges,

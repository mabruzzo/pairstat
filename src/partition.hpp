...
struct SlcStruct{ std::uint64_t start, stop; };
...
inline SlcStruct calc_chunk_slice(std::size_t chunk_index,
                                  std::size_t array_len, std::size_t num_chunks)
  noexcept
{
...
// start_B == stop_B == 0
struct StatTask{ std::uint64_t start_A, stop_A, start_B, stop_B; };
...
struct AutoSFPartitionStrat{
    ...
    noexcept
  {
    ...
    if (this->n_points <=1){ error("not enough points"); }
...
struct CrossSFPartitionStrat{
  ...
  ///     that prevents the user from subdividing the problem into partitions
  ///     that are too small
  static CrossSFPartitionStrat create(std::size_t nproc,
                                      std::size_t n_points_A,
...
using partition_variant = std::variant<AutoSFPartitionStrat,
                                       CrossSFPartitionStrat>;
...
public:
      ...
      next_index_2D_()
  {
    ...
    next_index_1D_ = 0;
    next_index_2D_ = {0, 0};
    while (next_index_1D_ < index_start_1D){ increment_index_(); }
  }
...
inline TaskIt* build_TaskIt_crossSF(std::uint64_t index_start_1D,
                                    ...
                                    std::uint64_t num_segments_B)
{
  CrossSFPartitionStrat tmp = {n_points_A, num_segments_A,
                               n_points_B, num_segments_B};
...
public:
  ...
  TaskItFactory(std::size_t nproc, std::size_t n_points,
    ...
    : nproc_(nproc),
      partition_strat_(TaskItFactory::build_strat_(nproc, n_points,
                                                   n_points_other,

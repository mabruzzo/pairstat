...
def get_left_edge(block_index, ds, compute_grid_slc = None,
                  cell_edge = True):
    """
    ...
    data_block_left_edge\
        = ds.index.grid_left_edge[tmp[block_index]].to('code_length').v
    ...
    if compute_grid_slc is None:
        ...
        if cell_edge:
            ...
            offset = 0.5
        for i in range(3):
            out[i] = ((compute_grid_slc[i].start + offset) * spacing[i] +

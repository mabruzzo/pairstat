...
///     creation properly (e.g. using placement-new)
template <int N_i64, int N_f64>
class StatDataView {
...
public:
  ...
  // destructor
  ~StatDataView() { if (data_ != nullptr) delete[] data_; }

  StatDataView(int n_registers, int n_i64, int n_f64)
    : n_registers_(n_registers), n_i64_(n_i64), n_f64_(n_f64)
  {
    require(n_registers > 0,
            "this constructor requires n_registers > 0");
  ...
  StatDataView<N_i64,N_f64>& operator=(const StatDataView<N_i64,N_f64>&)=delete;
  ...
  StatDataView<N_i64,N_f64>& operator=(StatDataView<N_i64,N_f64>&& o) {
    ...
    this->n_bytes_ = std::exchange(o.nbytes_, 0);
  ...
  void inplace_add(const StatDataView<N_i64,N_f64>& other) const {
    ...
    if ((N_i64 == -1) && (N_i64 == -1)) {
      ...
      error("we haven't implemented this branch yet!");
    }

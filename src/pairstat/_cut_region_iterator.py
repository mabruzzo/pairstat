...
def _neighbor_ind_iter(
    ...
    *,
    ...
    if (
        (not yield_batches)
        and (not force_traditional_neighbors)
        and all(e == 2 for e in subvol_decomp.subvols_per_ax[1:])
    ):
...
def _pos_quan_equan_arr_generator(
    data_region, sf_props, rand_generator=None, extra_quantities={}
):
    """
    ...
    for cut_region_index, cut_string in enumerate(sf_props.cut_regions):
        ...
        pos = np.array(
            [cad[ii].to(sf_props.dist_units).ndarray_view() for ii in ["x", "y", "z"]]
        )
        ...
        if npoints == 0:
            ...
            if max_points is not None and npoints > max_points:
                ...
                logging.info(f"Too many points (>{max_points}), discarding some")
                ipoints = rand_generator.permutation(npoints)[: sf_props.max_points]
                pos = pos[:, ipoints]
            else:
                ipoints = slice(None)
            ...
            for field in sf_props.quantity_components:
                tmp_l.append(
                    cad[field][ipoints].to(sf_props.quantity_units).ndarray_view()
                )
            ...
            for equan_name, (equan_units, _) in extra_quantities.items():
                equan_dict[equan_name] = cad[equan_name].to(equan_units).ndarray_view()
...
def _build_subvolume_cutstr(left_edge, right_edge):
    ...
    for i, ax in enumerate("xyz"):
        parts.append(f'(obj["gas", "{ax}"].to("{lunit}").ndarray_view() >= {lval[i]})')
        parts.append(f'(obj["gas", "{ax}"].to("{runit}").ndarray_view() <= {rval[i]})')
    return " & ".join(parts)
...
def subvolume_dataobjects(ds, subvol_indices, subvol_decomp):
    ...
    if len(subvol_indices) == 1:
        ...
        left_edge, right_edge = _get_subvol_left_right_edges(
            ds, subvol_decomp, subvol_indices[0]
        )
        ...
        expected = frozenset(
            product(
                range(_mins[0], _maxes[0] + 1),
                range(_mins[1], _maxes[1] + 1),
                range(_mins[2], _maxes[2] + 1),
            )
        )
        if (
            len(subvol_indices) != len(expected)
        ...
        # subvolumes
        box_left_edge, _ = _get_subvol_left_right_edges(ds, subvol_decomp, _mins)
        _, box_right_edge = _get_subvol_left_right_edges(ds, subvol_decomp, _maxes)
        box_region = _build_padded_box_region(ds, box_left_edge, box_right_edge)
...
class SimpleCutRegionIterBuilder:
    ...
    def __init__(
        self, ds, subvol_decomp, sf_props, extra_quantities={}, rand_generator=None
    ):
        self.ds = ds
    ...
    def __call__(self, subvol_index, is_central=False):
        ...
        """
        _, data_region = list(
            subvolume_dataobjects(self.ds, [subvol_index], self.subvol_decomp)
        )[0]
...
class EagerCutRegionIterBuilder(SimpleCutRegionIterBuilder):
    ...
    def clear_cache(self, keep_cur_center=False, run_gc=True):
        if self.cur_center is None or not keep_cur_center:
            ...
            for ind in self.neighbor_ind_iter(self.cur_center, self.subvol_decomp):
    ...
    def _build_iterators_for_batch(self, index_batch):
        ...
        index_region_pairs = subvolume_dataobjects(
            self.ds, index_batch, self.subvol_decomp
        )
        # TODO:
    ...
    def __call__(self, subvol_index, is_central=False):
        ...
        if is_central:  # construct the cached data for the new iterator
            ...
            if prev_center is not None:
                ...
                raise RuntimeError(
                    "Reusing this object for multiple central "
                    "subvol indices is not tested yet"
                )

            self._build_iterators_for_batch([self.cur_center])

            for batch in self.neighbor_ind_iter(
                self.cur_center, self.subvol_decomp, yield_batches=True
            ):

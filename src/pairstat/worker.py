...
def consolidate_partial_vsf_results(statistic, *rslts, stat_kw={}, dist_bin_edges=None):
    """
...
def _process_stat_choices(stat_kw_pairs, sf_params):
    ...
    stat_details = StatDetails(
        name_index_map={}, sf_stat_kw_pairs=[], nonsf_kernel_kw_pairs=[]
    )
...
class StatRsltContainer:
    def __init__(self, num_statistics, num_cut_regions):
        ...
        self._arr = np.empty((num_statistics, num_cut_regions), dtype=object)

    @classmethod
    def build_empty_container(cls, num_statistics, num_cut_regions):
        out = cls(num_statistics=num_statistics, num_cut_regions=num_cut_regions)
    ...
    def store_result(self, stat_index, cut_region_index, rslt):
        if self._arr[stat_index, cut_region_index] is not None:
            raise IndexError(
                "A result has already been stored for "
    ...
    def duplicate_results_for_cut_region(self, src_cr_index, dest_cr_index):
        ...
        for stat_ind in range(self.num_statistics):
            rslt = deepcopy(
                self.retrieve_result(stat_index=stat_ind, cut_region_index=src_cr_index)
            )
            self.store_result(
                stat_index=stat_ind, cut_region_index=dest_cr_index, rslt=rslt
            )
    ...
    def retrieve_result(self, stat_index, cut_region_index):
        if self.rslt_exists(stat_index, cut_region_index):
            ...
            raise ValueError(
                "There are no results to retrieve for "
                f"stat_index = {stat_index}, "
                f"cut_region_index = {cut_region_index}"
            )
...
class TaskResult:
    def __init__(
        ...
        if tmp.shape != (main_subvol_rslts.num_cut_regions,):
            raise ValueError(
                "main_subvol_available_points should be a 1D "
        ...
        # statistics
        assert main_subvol_rslts.num_statistics == consolidated_rslts.num_statistics
        assert main_subvol_rslts.num_cut_regions == consolidated_rslts.num_cut_regions
...
class SFWorker(_BaseWorker):
    ...
    def process_auto_stats(
        ...
        for tmp in cut_region_iter:
            ...
            if (
                cr_index == all_inclusive_cr_index
            ) and largest_cr_tracker.matches_max_num_points(available_points):
            ...
            with perf.region("auto-other"):  # calc non structure-func stats
                for kernel, kw in stat_details.nonsf_kernel_kw_pairs:
                    ...
                    if (available_points == 0) or (
                        ...
                        rslt = func(quan=quan, extra_quantities=extra_quan, kwargs=kw)
                    rslt_container.store_result(
                        stat_index=stat_index, cut_region_index=cr_index, rslt=rslt
                    )

    @staticmethod
    def process_cross_stats(
        cut_region_iter,
        ...
        for cr_index, o_pos, o_quan, o_eq, o_available_points in cut_region_iter:
            ...
            if (
                cr_index == all_inclusive_cr_index
            ) and largest_cr_tracker.matches_max_num_points(npoint_pair):
                # copy results from prior cut_region & skip the calculation
                rslt_container.duplicate_results_for_cut_region(
                    src_cr_index=largest_cr_tracker.max_size_cr_index,
                    dest_cr_index=cr_index,
            ...
            with perf.region("cross-other"):  # calc non structure-func stats
                for kernel, kw in stat_details.nonsf_kernel_kw_pairs:
                    ...
                    if not kernel.operate_on_pairs:
                        rslt_container.store_result(
                            stat_index=stat_index, cut_region_index=cr_index, rslt={}
                        )
                    else:
    ...
    def process_index(self, subvol_index):
        ...
        any_cross_stats = (len(stat_details.sf_stat_kw_pairs) > 0) or any(
            kernel.operate_on_pairs for kernel, _ in stat_details.nonsf_kernel_kw_pairs
        )
        ...
        for stat_ind, (stat_name, stat_kw) in enumerate(self.stat_kw_pairs):
            itr = enumerate(main_subvol_rslts.cut_region_iter(stat_index=stat_ind))
            for cut_region_i, main_subvol_rslt in itr:
                ...
                consolidated_rslts.store_result(
                    stat_index=stat_ind,
